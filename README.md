...
For images 0, 10 and 14 (the main menu, thief split and the end of clouds burst respectively), those are to be <font size ="3">***the first***</font> frame each occur; for the main menu its the first frame <font size ="3">**"Press START"**</font> is visible, for thief split it's the first frame of the cutscene that happens when you trigger the thief quest, and for the end of clouds burst its the first frame after you trigger the final barrel. The easiest way to obtain screenshots are by loading a previous PB in a video editor such as [VirtualDub2](https://sourceforge.net/projects/vdfiltermod/), going to the desired frame, and grabbing the screenshot from there.
